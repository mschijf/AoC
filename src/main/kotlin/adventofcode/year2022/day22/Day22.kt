...
class Day22(test: Boolean) : PuzzleSolverAbstract(test) {
    ...
    override fun resultPartOne(): String {
        val board = BoardDay22(inputLines.dropLast(2), test)
...
class BoardDay22(inputLines: List<String>, test: Boolean) {
    ...
    private var walker = this.Pos(0, 0).nextPos(Direction.RIGHT)
...
enum class Direction(val dRow: Int, val dCol: Int, val facingNumber: Int) {
    ...
    fun turn(turnChar: Char): Direction {
        return when (turnChar) {
            'R' -> values()[(this.ordinal + 1) % values().size]
            'L' -> values()[(this.ordinal + values().size - 1) % values().size]
            'O' -> this //360 graden
            'U' -> values()[(this.ordinal + 2) % values().size]
