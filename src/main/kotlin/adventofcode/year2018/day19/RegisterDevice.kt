...
class RegisterDevice(
    ...
    fun executeOperationList(operationList: List<Operation>, debug: Boolean=false, maxInstructions: Int = -1, ipStart:Int=0) {
        var operationExecuted = 0
        var ip = ipStart
        while (ip < operationList.size) {
            ...
            operationExecuted++
            if (maxInstructions > 0 && operationExecuted > maxInstructions)
                break
        }
    }
